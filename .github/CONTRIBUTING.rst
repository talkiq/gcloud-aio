...
autogenerated from the former in CI. You may need to keep this in mind while
developing; all code will need to be generateable into both module trees and
runnable under both Python 2 and 3 (though there are some conversion scripts
we run to make things a bit more ergonomic -- you're generally safe to write
Python 3 asyncio code and rely on the conversation script). Feel free to ping a
maintainer if you're having issues with this!
